 ...
 * @author Nu Echo Inc.
 */
public final class VoiceXmlInputTurnFactory
    implements InputTurnFactory<VoiceXmlInputTurn, VoiceXmlFirstTurn> {
  ...
  public VoiceXmlInputTurn createInputTurn(HttpServletRequest request, HttpServletResponse response)
    ...
    if (result == null)
      throw new InputTurnFactoryException(
          "Unable to process request. Missing '" + INPUT_TURN_PARAMETER + "' parameter.");
  ...
  private static void addEvents(JsonObject resultObject, VoiceXmlInputTurn voiceXmlInputTurn) {
    if (!resultObject.containsKey(EVENTS_PROPERTY))
      return;
  ...
  private static void addObject(JsonObject resultObject, VoiceXmlInputTurn voiceXmlInputTurn) {
    if (!resultObject.containsKey(VALUE_PROPERTY))
      return;
  ...
  private static void addTransferStatusInfo(JsonObject resultObject,
      VoiceXmlInputTurn voiceXmlInputTurn) {
    if (!resultObject.containsKey(TRANSFER_PROPERTY))
      return;

    JsonObject transferResultObject = resultObject.getJsonObject(TRANSFER_PROPERTY);

    TransferStatus transferStatus =
        new TransferStatus(transferResultObject.getString(TRANSFER_STATUS_PROPERTY));
  ...
  private static void addRecordingInfo(JsonObject resultObject, VoiceXmlInputTurn voiceXmlInputTurn,
      Map<String, FileUpload> files) {
    if (!resultObject.containsKey(RECORDING_META_DATA_PROPERTY))
      return;
  ...
  private void addRecognitionInfo(JsonObject jsonObject, VoiceXmlInputTurn voiceXmlInputTurn) {
    ...
    if (!jsonObject.containsKey(RECOGNITION_PROPERTY))
      return;
    ...
    if (recognitionObject.containsKey(MARK_PROPERTY)) {
      ...
      markInfo = new MarkInfo(markObject.getString(MARK_NAME_PROPERTY),
          Duration.milliseconds(timeInMilliseconds));
    }
  ...
  private void processRequestParametersAndFiles(HttpServletRequest request,
      Map<String, String> parameters, Map<String, FileUpload> files)
      throws InputTurnFactoryException {
    if (JakartaServletFileUpload.isMultipartContent(request)) {
      ...
      try {
        ...
        while (itemIterator.hasNext()) {
          FileItemInput fileItemStream = itemIterator.next();

          byte[] bytes;
          String parameterName = fileItemStream.getFieldName();
          try {
            InputStream stream = fileItemStream.getInputStream();
          ...
          if (!fileItemStream.isFormField()) {
            FileUpload fileUpload = new FileUpload(fileItemStream.getName(),
                fileItemStream.getContentType(), bytes, getHeaders(fileItemStream));
            ...
            try {
              ...
              throw new Exception(
                  "Unable to find encoding '" + encoding + "'. Cannot decode text form field '"
                      + parameterName + "' of multipart request.",
                  exception);
            }
          }
  ...
  private String findEncoding(HttpServletRequest request, FileItemInput fileItemStream) {
  ...
  private Map<String, String> getHeaders(FileItemInput fileItemStream) {
