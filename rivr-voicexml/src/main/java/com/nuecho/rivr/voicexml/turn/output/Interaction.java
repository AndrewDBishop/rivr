...
/**
 * An {@link Interaction} is a {@link VoiceXmlOutputTurn} that represents a list of {@link Prompt}
 * with an optional {@link FinalRecognitionWindow} or {@link FinalRecordingWindow} phase.
 * <p>
 * Each {@link Interaction.Prompt} represents a phase of the interaction with a sequence of
 * {@link AudioItem} and optional speech/DTMF recognition configurations.
 *
 * @author Nu Echo Inc.
...
public class Interaction extends VoiceXmlOutputTurn {
   ...
   * @param name The name of this turn. Not empty.
   * @param prompts The list of {@link Prompt}. Not null.
   * @param finalRecognitionWindow The final recognition phase configuration. Not null.
   */
  public Interaction(String name, List<Prompt> prompts,
      FinalRecognitionWindow finalRecognitionWindow) {
    super(name);
    Assert.notNull(prompts, "prompts");
   ...
   * @param name The name of this turn. Not empty.
   * @param prompts The list of {@link Prompt}. Not null.
   * @param finalRecordingWindow The final recording phase configuration. Not null.
   */
  ...
  @Override
  protected void fillVoiceXmlDocument(Document document, Element formElement,
      VoiceXmlDialogueContext dialogueContext) throws VoiceXmlDocumentRenderingException {
    ...
    for (int interactionPromptIndex = 0; interactionPromptIndex < mPrompts
        .size(); interactionPromptIndex++) {
      ...
      boolean usingField;
      boolean bargeIn =
          prompt.getDtmfRecognition() != null || prompt.getSpeechRecognition() != null;
      ...
      processDtmfRecognition(getLocalDtmfRecognition(dtmfRecognition, dtmfGlobalRecognition),
          formItemElement);

      processSpeechRecognition(
          getLocalSpeechRecognition(speechRecognition, speechGlobalRecognition), formItemElement);

      if (usingField) {
        if (mFinalRecognitionWindow != null && interactionPromptIndex == mPrompts.size() - 1
            && same(mFinalRecognitionWindow.getDtmfRecognition(), prompt.getDtmfRecognition())
            && same(mFinalRecognitionWindow.getSpeechRecognition(),
                prompt.getSpeechRecognition())) {
          addDurationProperty(formItemElement, TIMEOUT_PROPERTY,
    ...
    if (mFinalRecognitionWindow != null) {
      if (!recognitionMergedWithLastPrompt) {
        ...
        processDtmfRecognition(getLocalDtmfRecognition(mFinalRecognitionWindow.getDtmfRecognition(),
            dtmfGlobalRecognition), recognitionFormItemElement);

        processSpeechRecognition(
            getLocalSpeechRecognition(mFinalRecognitionWindow.getSpeechRecognition(),
                speechGlobalRecognition),
            recognitionFormItemElement);

        addDurationProperty(recognitionFormItemElement, TIMEOUT_PROPERTY,
            mFinalRecognitionWindow.getNoInputTimeout());
      }
      ...
      addDurationProperty(recordingFormItemElement, TIMEOUT_PROPERTY,
          mFinalRecordingWindow.getNoInputTimeout());
      setDurationAttribute(recordingFormItemElement, FINAL_SILENCE_ATTRIBUTE,
      ...
      List<? extends AudioItem> acknowledgeAudioItems =
          mFinalRecordingWindow.getAcknowledgeAudioItems();
  ...
  private void addRecordingResultHandlerScript(Recording recording, Element parent) {
    createScript(parent, RIVR_SCOPE_OBJECT + ".addRecordingResult(dialog." + RECORD_FORM_ITEM_NAME
        + ", dialog." + RECORD_FORM_ITEM_NAME + "$, " + recording.isPostAudioToServer() + ");");

  }

  private static void renderPrompts(Prompt prompt, Element parentElement,
      VoiceXmlDialogueContext voiceXmlDialogueContext, boolean bargeIn)
  ...
  private SpeechRecognition factorizeGlobalSpeechRecognition() {
    ...
    if (speechConfigurations.isEmpty())
      return null;
    ...
    for (SpeechRecognition speechConfiguration : speechConfigurations) {
      ...
      if (!same(configuration.getIncompleteTimeout(),
          speechGlobalRecognition.getIncompleteTimeout())) {
      ...
      if (!same(configuration.getMaxSpeechTimeout(),
          speechGlobalRecognition.getMaxSpeechTimeout())) {
      ...
      if (!same(configuration.getSpeedVersusAccuracy(),
          speechGlobalRecognition.getSpeedVersusAccuracy())) {
  ...
  private DtmfRecognition factorizeGlobalDtmfRecognition() {
    ...
    for (DtmfRecognition dtmfRecognition : dtmfRecognitions) {
      ...
      if (!same(configuration.getInterDigitTimeout(),
          dtmfGlobalRecognition.getInterDigitTimeout())) {
  ...
  private static void factorizeParameters(Recognition globalRecognition,
      Recognition localRecognition) {
    for (String propertyName : globalRecognition.getPropertyNames()) {
      if (!same(localRecognition.getProperty(propertyName),
          globalRecognition.getProperty(propertyName))) {
        globalRecognition.removeProperty(propertyName);
      }
    }
  }

  private static void addBargeIn(Prompt prompt, DtmfRecognition dtmfRecognition,
      SpeechRecognition speechRecognition, Element formItemElement) {
  ...
  private static void removeGlobalParameters(Recognition globalRecognition,
      Recognition localRecognition) {
    for (String propertyName : globalRecognition.getPropertyNames()) {
  ...
  private void createFormLevelFilled(Element parent) throws VoiceXmlDocumentRenderingException {
    ...
    if (mFinalRecognitionWindow != null) {
      List<? extends AudioItem> acknowledgeAudioItems =
          mFinalRecognitionWindow.getAcknowledgeAudioItems();
      if (!acknowledgeAudioItems.isEmpty()) {
  ...
  public enum BargeInType {
    /**
     * Normal speech-detection barge-in. Triggers only when speech is detected.
     */
    speech,
  ...
  /**
   * A {@link FinalRecognitionWindow} is an optional final phase of an {@link Interaction}.
   * <p>
   * It specifies a recognition configuration, and optionally, a no input timeout and a sequence of
   * {@link AudioItem} that is played if a recognition is successful.
   *
   * @author Nu Echo Inc.
   */
  public static final class FinalRecognitionWindow implements JsonSerializable {
    ...
    public FinalRecognitionWindow(DtmfRecognition dtmfRecognition,
        SpeechRecognition speechRecognition, Duration noInputTimeout) {
    ...
    public JsonValue asJson() {
      ...
      JsonUtils.add(builder, ACKNOWLEDGE_AUDIO_ITEMS_PROPERTY,
          JsonUtils.toJson(mAcknowledgeAudioItems));
    ...
    public int hashCode() {
      ...
      int result = 1;
      result = prime * result
          + ((mAcknowledgeAudioItems == null) ? 0 : mAcknowledgeAudioItems.hashCode());
    ...
    public boolean equals(Object obj) {
      ...
      if (mAcknowledgeAudioItems == null) {
        if (other.mAcknowledgeAudioItems != null)
          return false;
      } else if (!mAcknowledgeAudioItems.equals(other.mAcknowledgeAudioItems))
        return false;
      if (mDtmfRecognition == null) {
        if (other.mDtmfRecognition != null)
          return false;
      } else if (!mDtmfRecognition.equals(other.mDtmfRecognition))
        return false;
      if (mNoInputTimeout == null) {
        if (other.mNoInputTimeout != null)
          return false;
      } else if (!mNoInputTimeout.equals(other.mNoInputTimeout))
        return false;
      if (mSpeechRecognition == null) {
        if (other.mSpeechRecognition != null)
          return false;
      } else if (!mSpeechRecognition.equals(other.mSpeechRecognition))
        return false;
      return true;
    }
  ...
  /**
   * A {@link FinalRecordingWindow} is an optional final phase of an {@link Interaction}.
   * <p>
   * It specifies a recording configuration, and optionally, a no input timeout and a sequence of
   * {@link AudioItem} that is played if a recording is successful.
   *
   * @author Nu Echo Inc.
   */
  public static final class FinalRecordingWindow implements JsonSerializable {
    ...
    public boolean equals(Object obj) {
      ...
      if (mNoInputTimeout == null) {
        ...
        return false;
      if (mRecording == null) {
        if (other.mRecording != null)
          return false;
      } else if (!mRecording.equals(other.mRecording))
        return false;
      return true;
    }
  ...
  /**
   * A {@link Prompt} represent a phase in an {@link Interaction} and is composed of a sequence of
   * {@link AudioItem} and optionally a speech and/or a DTMF recognition configuration.
   *
   * @author Nu Echo Inc.
   */
  public static final class Prompt implements JsonSerializable {
    ...
    /**
     * @param speechRecognition The speech recognition configuration. Optional.
     * @param dtmfRecognition The DTMF recognition configuration. Optional.
     * @param audioItems The list of {@link AudioItem}. Not null.
     */
    public Prompt(SpeechRecognition speechRecognition, DtmfRecognition dtmfRecognition,
    ...
    /**
     * @param language language code for this prompt. <code>null</code> if language should be reset
     *        to platform-specific default value for the prompts to be added.
     */
    ...
    /**
     * @param bargeInType {@link BargeInType#speech} or {@link BargeInType#hotword}.
     *        <code>null</code> if language should be reset to platform-specific default value for
     *        the prompts to be added.
     */
    ...
    public JsonValue asJson() {
      ...
      JsonUtils.add(builder, BARGE_IN_TYPE_PROPERTY,
          mBargeInType == null ? null : mBargeInType.name());
    ...
    public boolean equals(Object obj) {
      ...
      if (mAudioItems == null) {
        if (other.mAudioItems != null)
          return false;
      } else if (!mAudioItems.equals(other.mAudioItems))
        return false;
      if (mBargeInType != other.mBargeInType)
        return false;
      if (mDtmfRecognition == null) {
        if (other.mDtmfRecognition != null)
          return false;
        ...
        return false;
      if (mLanguage == null) {
        if (other.mLanguage != null)
          return false;
      } else if (!mLanguage.equals(other.mLanguage))
        return false;
      if (mSpeechRecognition == null) {
        if (other.mSpeechRecognition != null)
          return false;
   ...
   * <li>Add some prompts
   * <ul>
   * <li>For each prompt, speech/DTMF recognition can be specified (thus enabling <i>barge-in</i>
   * when the prompt is played).
   * </ul>
   * <li>Once all prompts are added, optionally specify either:
   * <ul>
   * <li>a final recognition window (speech or DTMF)
   * <li>a final recording window
   * </ul>
   * </ul>
   * <p>
   * At any time, it is possible to change the current language used for prompts (relevant to speech
   * synthesis) and the barge-in type, i.e. <i>speech</i> or <i>hotword</i>.
   * <p>
  ...
  public static final class Builder {
    ...
    /**
     * Sets the barge-in type to either {@link BargeInType#speech} or {@link BargeInType#hotword}
     * for the prompts that will be added using one of the <code>addPrompt(...)</code> methods.
     * <p>
     * Note: When a {@link Builder} is created, the default value for this flag is
     * <code>null</code>, meaning the barge-in type will be platform-dependant.
     *
     * @param bargeInType
     *        <ul>
     *        <li>{@link BargeInType#speech}
     *        <li>{@link BargeInType#hotword}
     *        <li><code>null</code> reverts to platform default value
     *        </ul>
     * @return this builder
     */
    ...
    /**
     * Sets the language code for the prompts that will be added using one of the
     * <code>addPrompt(...)</code> methods.
     * <p>
     * Note: When a @{link Builder} is created, the default value for this property is
     * <code>null</code>, i.e. no explicit language code will be generated in the VoiceXML thus
     * relying on the platform-specific default language code.
     *
     * @param language language code for this prompt. <code>null</code> if language should be reset
     *        to platform-specific default value for the prompts to be added.
     * @return this builder
     ...
     * @return this builder
     */
    public Builder addPrompt(SpeechRecognition speechRecognition,
        List<? extends AudioItem> audioItems) {
     ...
     *
     * @param speechRecognition configuration for the speech recognition or <code>null</code> to
     *        disable DTMF recognition.
     * @param dtmfRecognition configuration for the DTMF recognition or <code>null</code> to disable
     *        DTMF recognition.
     * @param audioItems audio items to be played during this prompt.
     * @return this builder
     */
    public Builder addPrompt(DtmfRecognition dtmfRecognition, SpeechRecognition speechRecognition,
        AudioItem... audioItems) {
    ...
    /**
     * Builds the interaction. This method adds a DTMF recognition window after the prompts.
     *
     * @param dtmfRecognition configuration for the DTMF recognition
     * @param noinputTimeout timeout value before a <code>noinput</code> is generated.
     * @param acknowledgeAudioItems audioItems to be played upon recognition
     * @return the interaction
     */
    public Interaction build(DtmfRecognition dtmfRecognition, Duration noinputTimeout,
        AudioItem... acknowledgeAudioItems) {
      return build(dtmfRecognition, null, noinputTimeout, asListChecked(acknowledgeAudioItems));
    }

    /**
     * Builds the interaction. This method adds a speech recognition window after the prompts.
     *
     * @param speechRecognition configuration for the speech recognition
     * @param noinputTimeout timeout value before a <code>noinput</code> is generated.
     * @param acknowledgeAudioItems audioItems to be played upon recognition
     * @return the interaction
     */
    public Interaction build(SpeechRecognition speechRecognition, Duration noinputTimeout,
        AudioItem... acknowledgeAudioItems) {
    ...
    /**
     * Builds the interaction. This method adds a speech and DTMF recognition window after the
     * prompts.
     *
     * @param speechRecognition configuration for the speech recognition
     * @param dtmfRecognition configuration for the DTMF recognition
     ...
     * @return the interaction
     */
    public Interaction build(DtmfRecognition dtmfRecognition, SpeechRecognition speechRecognition,
        Duration noinputTimeout, AudioItem... acknowledgeAudioItems) {
      return build(dtmfRecognition, speechRecognition, noinputTimeout,
          asListChecked(acknowledgeAudioItems));
    }
     ...
     */
    public Interaction build(DtmfRecognition dtmfRecognition, SpeechRecognition speechRecognition,
        Duration noinputTimeout, List<? extends AudioItem> acknowledgeAudioItems) {
      ...
      checkBuilt();
      FinalRecognitionWindow finalRecognitionWindow =
          new FinalRecognitionWindow(dtmfRecognition, speechRecognition, noinputTimeout);
    ...
    /**
     * Builds the interaction. This method adds a recording window after the prompts.
     *
     * @param recording configuration for the recording
     * @param noinputTimeout timeout value before a <code>noinput</code> is generated.
     * @param acknowledgeAudioItems audioItems to be played upon recording completion
     * @return the interaction
     */
    public Interaction build(Recording recording, Duration noinputTimeout,
        AudioItem... acknowledgeAudioItems) {
    ...
    public Interaction build(Recording recording, Duration noinputTimeout,
      ...
      checkBuilt();
      FinalRecordingWindow finalRecordingWindow =
          new FinalRecordingWindow(recording, noinputTimeout);
    ...
    /**
     * Builds the interaction. This method does not allow any recording nor recognition after the
     * prompts.
     *
     * @return the interaction
     */
    ...
    private void checkBuilt() {
      if (mBuilt)
        throw new IllegalStateException("build() method was already called.");
      mBuilt = true;
    }
  }
