...
public final class VoiceXmlDomUtil {
  ...
  public static final String RESULT_RECORDING_METADATA_SCOPE_OBJECT =
      RIVR_INPUT_TURN_SCOPE_OBJECT + ".recordingMetaData";
  public static final String RESULT_RECORDING_METADATA_DATA_SCOPE_OBJECT =
      RESULT_RECORDING_METADATA_SCOPE_OBJECT + ".data";
  ...
  public static Element createVoiceXmlDocumentRoot(String language) {
    ...
    vxmlElement.setAttributeNS(XMLConstants.XMLNS_ATTRIBUTE_NS_URI, XMLConstants.XMLNS_ATTRIBUTE,
        VOICEXML_NAMESPACE);
  ...
  public static Document createDocument(VoiceXmlDialogueContext voiceXmlDialogueContext) {
    ...
    String rootDocumentPath =
        voiceXmlDialogueContext.getContextPath() + voiceXmlDialogueContext.getServletPath()
            + VoiceXmlDialogueServlet.ROOT_PATH + voiceXmlDialogueContext.getDialogueId();

    vxmlElement.setAttribute(APPLICATION_ATTRIBUTE, rootDocumentPath);

    StringBuilder script = new StringBuilder();
    script.append(RIVR_SCOPE_OBJECT).append(".").append(LOCAL_ERROR_HANDLING_PROPERTY).append(" = ")
        .append(FALSE).append("; ");
  ...
  private static void processProperties(VoiceXmlDialogueContext voiceXmlDialogueContext,
      Element vxmlElement) {
  ...
  private static void processFetchRendering(VoiceXmlDialogueContext voiceXmlDialogueContext,
      Element vxmlElement) {
    DefaultFetchConfiguration defaultFetchConfiguration =
        voiceXmlDialogueContext.getFetchConfiguration();
    ...
    addDurationProperty(vxmlElement, FETCH_TIMEOUT_PROPERTY,
  ...
  public static void applyRessourceFetchConfiguration(Element element,
    ...
    if (fetchConfiguration != null) {
      ...
      setCacheControlDurationAttribute(element, MAX_STALE_ATTRIBUTE,
  ...
  private static void applyResourceDefaultFetchConfiguration(Element parent,
    ...
    if (fetchConfiguration != null) {
      ...
      addFetchHintProperty(parent, resourceType + FETCH_HINT_PROPERTY_SUFFIX,
          fetchConfiguration.getFetchHint());
      addCacheControlDurationProperty(parent, resourceType + MAX_AGE_PROPERTY_SUFFIX,
          fetchConfiguration.getMaxAge());
      addCacheControlDurationProperty(parent, resourceType + MAX_STALE_PROPERTY_SUFFIX,
          fetchConfiguration.getMaxStale());
    }
  }

  public static void createPrompt(String language, Element parent,
  ...
  public static void processAudioItems(List<? extends AudioItem> audioItems, Element promptElement)
    ...
    for (AudioItem audioItem : audioItems) {
      if (audioItem instanceof AudioFile) {
        ...
        throw new VoiceXmlDocumentRenderingException(
            "Cannot handle prompt rendering element of type '" + audioItem.getClass() + "'");
    }
  }

  public static void addFetchHintProperty(Element parent, String propertyName,
      FetchHint fetchHint) {
    if (fetchHint != null) {
  ...
  public static void setFetchHintAttribute(Element parent, String propertyName,
      FetchHint fetchHint) {
    if (fetchHint != null) {
  ...
  public static void addCacheControlDurationProperty(Element parent, String propertyName,
      Duration value) {
    if (value != null) {
  ...
  public static void setCacheControlDurationAttribute(Element parent, String propertyName,
      Duration value) {
    if (value != null) {
  ...
  private static Element createSubmitElement(Element parent,
      VoiceXmlDialogueContext voiceXmlDialogueContext, SubmitMethod method,
      VoiceXmlOutputTurn outputTurn, String... nameList) {
    Element submitElement = DomUtils.appendNewElement(parent, SUBMIT_ELEMENT);
    submitElement.setAttribute(NEXT_ATTRIBUTE,
        getSubmitPathForTurn(voiceXmlDialogueContext, outputTurn));
  ...
  public static String getSubmitPathForTurn(VoiceXmlDialogueContext voiceXmlDialogueContext,
    ...
    return getSubmitPath(voiceXmlDialogueContext, voiceXmlDialogueContext.getTurnIndex(),
        outputTurn.getName());
  }

  public static String getSubmitPath(VoiceXmlDialogueContext voiceXmlDialogueContext, int turnIndex,
      String turnName) {
    return getServletPathWithSessionId(voiceXmlDialogueContext) + "/" + turnIndex + "/"
        + StringUtils.replaceAll(URI_PATH_REPLACEMENT_CHAR, turnName, "_");
  }

  public static String getServletPathWithSessionId(
      VoiceXmlDialogueContext voiceXmlDialogueContext) {
    return voiceXmlDialogueContext.getContextPath() + voiceXmlDialogueContext.getServletPath() + "/"
  ...
  public static void applyDocumentFetchConfiguration(Element submitElement,
    ...
    DocumentFetchConfiguration documentFetchConfiguration =
        defaultFetchConfiguration.getDocumentFetchConfiguration();
  ...
  public static void applyDocumentFetchConfiguration(Element submitElement,
    ...
    if (submitTurnFetchConfiguration == null)
      return;
  ...
  public static void addSubmitForm(VoiceXmlDialogueContext dialogueContext, Document document,
    ...
    createVarElement(blockElement, INPUT_TURN_VARIABLE,
        RIVR_SCOPE_OBJECT + ".toJson(" + RIVR_INPUT_TURN_SCOPE_OBJECT + ")");

    Element ifElement = DomUtils.appendNewElement(blockElement, IF_ELEMENT);
    ifElement.setAttribute(COND_ATTRIBUTE,
        RIVR_SCOPE_OBJECT + ".hasRecording(" + RIVR_INPUT_TURN_SCOPE_OBJECT + ")");
    ...
    Element submitElement = createSubmitElement(ifElement, dialogueContext, SubmitMethod.post, turn,
        INPUT_TURN_VARIABLE, RECORDING_VARIABLE);
    submitElement.setAttribute(ENCTYPE_ATTRIBUTE, MULTIPART_FORM_DATA);
    DomUtils.appendNewElement(ifElement, ELSE_ELEMENT);

    submitElement = createSubmitElement(ifElement, dialogueContext, SubmitMethod.post, turn,
        INPUT_TURN_VARIABLE);
  }

  public static void addFatalErrorHandlerForm(VoiceXmlDialogueContext dialogueContext,
      Document document, VoiceXmlDocumentTurn turn) {
    Element vxmlElement = document.getDocumentElement();
  ...
  public static void processDtmfRecognition(DtmfRecognition dtmfRecognition,
      Element formItemElement) throws VoiceXmlDocumentRenderingException {
    if (dtmfRecognition == null)
      return;

    VoiceXmlDomUtil.renderGrammars(dtmfRecognition.getGrammarItems(), formItemElement,
        DTMF_INPUT_MODE);

    addDurationProperty(formItemElement, INTER_DIGIT_TIMEOUT_PROPERTY,
  ...
  public static void processSpeechRecognition(SpeechRecognition speechRecognition,
      Element formItemElement) throws VoiceXmlDocumentRenderingException {
    if (speechRecognition == null)
      return;

    VoiceXmlDomUtil.renderGrammars(speechRecognition.getGrammarItems(), formItemElement,
        VOICE_INPUT_MODE);

    addDurationProperty(formItemElement, COMPLETE_TIMEOUT_PROPERTY,
        speechRecognition.getCompleteTimeout());

    addDurationProperty(formItemElement, INCOMPLETE_TIMEOUT_PROPERTY,
        speechRecognition.getIncompleteTimeout());

    addDurationProperty(formItemElement, MAX_SPEECH_TIMEOUT_PROPERTY,
        speechRecognition.getMaxSpeechTimeout());

    VoiceXmlDomUtil.addNumberProperty(formItemElement, MAX_N_BEST_PROPERTY,
        speechRecognition.getMaxNBest());

    VoiceXmlDomUtil.addNumberProperty(formItemElement, CONFIDENCE_LEVEL_PROPERTY,
        speechRecognition.getConfidenceLevel());

    VoiceXmlDomUtil.addNumberProperty(formItemElement, SENSITIVITY_PROPERTY,
        speechRecognition.getSensitivity());

    VoiceXmlDomUtil.addNumberProperty(formItemElement, SPEED_VERSUS_ACCURACY_PROPERTY,
  ...
  public static void renderGrammars(List<? extends GrammarItem> grammarItems, Element parent,
      String mode) throws VoiceXmlDocumentRenderingException {
    for (GrammarItem grammarItem : grammarItems) {
      ...
      if (grammarItem instanceof GrammarReference) {
        ...
        throw new AssertionError("Unsupported grammar item: " + grammarItem);
      ...
      if (weight != null) {
        ...
        grammarElement.setAttribute(WEIGHT_ATTRIBUTE,
            weigthDecimalFormat.format(weightDoubleValue));
      }
  ...
  public static void renderInlineStringGrammar(Element grammarElement,
  ...
  public static void renderGrammarReference(GrammarReference grammarReference,
      Element grammarElement) {
  ...
  public static void addNamelist(Element parentElement, Element namelistHolderElement,
