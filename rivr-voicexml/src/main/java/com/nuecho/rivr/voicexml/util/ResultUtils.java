...
/**
 * Utility class containing helper methods for result returned in some {@link VoiceXmlLastTurn last
 * turns}.
 *
 * @author Nu Echo Inc.
 */
public class ResultUtils {
  ...
  private static void addThrowable(JsonArrayBuilder arrayBuilder, Throwable throwable,
      Throwable parent) {
    ...
    if (parent != null) {
      ...
      while (thisStackTraceIndex >= 0 && otherStackTraceIndex >= 0
    ...
    for (int index = 0; index < maxFrameIndex; index++) {
      ...
      stackTraceBuilder
          .add(stackTraceElement.getClassName() + "." + stackTraceElement.getMethodName() + "("
