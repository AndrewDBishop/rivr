...
/**
 * Implementation of the {@link DialogueServlet} specialized for VoiceXML. This servlet handles
 * requests from the VoiceXML platform and responds with VoiceXML documents. It also intercepts
 * special resources ( <code>/script</code> and <code>/root</code>).
 * <h2>init args</h2>
 * <p>
 * The following servlet initial arguments are supported:
 * <dl>
 * <dt>com.nuecho.rivr.voicexml.errorHandler.class</dt>
 * <dd>Class name of the error handlder. This class must implements {@link VoiceXmlErrorHandler}, be
 * public and non-abstract and have a public no-argument constructor. Default:
 * <code>com.nuecho.rivr.voicexml.servlet.DefaultErrorHandler</code></dd>
 * <dt>com.nuecho.rivr.voicexml.errorHandler.key</dt>
 * <dd>As an alternative to <code>com.nuecho.rivr.voicexml.errorHandler.class</code>, this indicates
 * the servlet context attribute name under which the {@link VoiceXmlErrorHandler} can be found.
 * Default: (none: an instance of <code>com.nuecho.rivr.voicexml.servlet.DefaultErrorHandler</code>
 * is used)</dd>
 * <dt>com.nuecho.rivr.voicexml.dialogueFactory.class</dt>
 * <dd>Class name of the dialogue factory. This class must implements
 * {@link VoiceXmlDialogueFactory}, be public and non-abstract and have a public no-argument
 * constructor. Default: (none).</dd>
 * <dt>com.nuecho.rivr.voicexml.dialogueFactory.key</dt>
 * <dd>As an alternative to <code>com.nuecho.rivr.voicexml.dialogueFactory.class</code>, this
 * indicates the servlet context attribute name under which the {@link VoiceXmlDialogueFactory} can
 * be found. Default: (none)</dd>
 * <dt>com.nuecho.rivr.voicexml.dialogue.class</dt>
 * <dd>If neither <code>com.nuecho.rivr.voicexml.dialogueFactory.class</code> nor
 * <code>com.nuecho.rivr.voicexml.dialogueFactory.key</code> is specified, this specifies the class
 * name of the dialogue. This class must implements {@link VoiceXmlDialogue}, be public and
 * non-abstract and have a public no-argument constructor. Default: (none).</dd>
 ...
 * <code>com.nuecho.rivr.voicexml.dialogue.class</code> is specified, this specifies the servlet
 * context attribute name under which the {@link VoiceXmlDialogue} can be found. Default:
 * (none)</dd>
 * <dt>com.nuecho.rivr.voicexml.loggerFactory.class</dt>
 * <dd>Class name of the dialogue factory. This class must implements {@link LoggerFactory
 * org.slf4j.LoggerFactory}, be public and non-abstract and have a public no-argument constructor.
 * Default: (none: {@link org.slf4j.LoggerFactory#getILoggerFactory()} is used as the logger
 * factory).</dd>
 * <dt>com.nuecho.rivr.voicexml.loggerFactory.key</dt>
 * <dd>As an alternative to <code>com.nuecho.rivr.voicexml.loggerFactory.class</code>, this
 * indicates the servlet context attribute name under which the {@link LoggerFactory
 * org.slf4j.LoggerFactory} can be found. Default: (none:
 * {@link org.slf4j.LoggerFactory#getILoggerFactory()} is used as the logger factory).</dd>
 * </dl>
 * <p>
 * <b>Important:</b> one of the following must be specified, they are mutually exclusive:
 * <ul>
 ...
 * @author Nu Echo Inc.
 */
public class VoiceXmlDialogueServlet extends
  ...
  private static final String INITIAL_ARGUMENT_ERROR_HANDLER =
      INITIAL_ARGUMENT_PREFIX + "errorHandler";
  private static final String INITIAL_ARGUMENT_DIALOGUE_FACTORY =
      INITIAL_ARGUMENT_PREFIX + "dialogueFactory";
  private static final String INITIAL_ARGUMENT_DIALOGUE = INITIAL_ARGUMENT_PREFIX + "dialogue";
  private static final String INITIAL_ARGUMENT_LOGGER_FACTORY =
      INITIAL_ARGUMENT_PREFIX + "loggerFactory";
  ...
  private VoiceXmlRootDocumentFactory mRootDocumentFactory =
      new DefaultVoiceXmlRootDocumentFactory();
  ...
  private void initializeProperties() throws DialogueServletInitializationException {
    ...
    VoiceXmlErrorHandler errorHandler =
        find(INITIAL_ARGUMENT_ERROR_HANDLER, VoiceXmlErrorHandler.class);
    ...
    if (className != null) {
      try {
        ...
        if (!VoiceXmlDialogue.class.isAssignableFrom(rawDialogueClass)) {
          String message = "Dialogue class " + rawDialogueClass + " does not implements "
              + VoiceXmlDialogue.class.getName() + ".";
          throw new DialogueServletInitializationException(message);
        }

        @SuppressWarnings("unchecked")
        Class<? extends VoiceXmlDialogue> dialogueClass =
            (Class<? extends VoiceXmlDialogue>) rawDialogueClass;
        ...
        throw new DialogueServletInitializationException("Cannot initialize dialogue factory.",
            exception);
      }
    } else if (key != null) {
  ...
  protected VoiceXmlDialogueFactory getDialogueFactory()
  ...
  private <T> T findInServletContext(String servletContextKey, Class<T> type, String item)
    ...
    if (object == null)
      throw new DialogueServletInitializationException(
          "Cannot find " + item + " with name [" + servletContextKey + "] in servlet context.");

    if (!(type.isInstance(object)))
      throw new DialogueServletInitializationException(
          "Servlet context object [" + servletContextKey + "] does not implements " + type.getName()
              + ". Actual class is " + object.getClass().getName());
  ...
  private <T> T instantiate(String className, Class<T> type, String item)
    ...
    try {
      ...
      return type.cast(classObject.getDeclaredConstructor().newInstance());
    } catch (ClassNotFoundException exception) {
      throw new DialogueServletInitializationException(
          "Cannot find " + item + " class '" + className + "'", exception);
    } catch (InstantiationException exception) {
      throw new DialogueServletInitializationException(
          "Cannot instantiate " + item + " of class '" + className + "'", exception);
    } catch (IllegalAccessException exception) {
      throw new DialogueServletInitializationException(
          "Cannot access " + item + " class '" + className + "'", exception);
    } catch (IllegalArgumentException exception) {
      throw new DialogueServletInitializationException(
          "Illegal Arg:" + item + " class '" + className + "'", exception);
    } catch (InvocationTargetException exception) {
      throw new DialogueServletInitializationException(
          "Target problem:" + item + " class '" + className + "'", exception);
    } catch (NoSuchMethodException exception) {
      throw new DialogueServletInitializationException(
          "Servlet exception:" + item + " class '" + className + "'", exception);
    } catch (SecurityException exception) {
      throw new DialogueServletInitializationException(
          "Security problem: " + item + " class '" + className + "'", exception);
    }
  }

  @Override
  protected StepRenderer<VoiceXmlInputTurn, VoiceXmlOutputTurn, VoiceXmlLastTurn, VoiceXmlDialogueContext> getStepRenderer(
    ...
    if (acceptHeader.indexOf("application/json") != -1)
      return mJsonStepRenderer;
    if (acceptHeader.indexOf("application/javascript") != -1)
      return mJsonStepRenderer;
    if (acceptHeader.indexOf("application/voicexml+xml") != -1)
      return mVoiceXmlStepRenderer;
    return mVoiceXmlStepRenderer;
  }

  private void processRessource(HttpServletRequest request, HttpServletResponse response,
      String pathInfo) throws IOException, ServletException {
    InputStream inputStream =
        VoiceXmlDialogueServlet.class.getResourceAsStream(pathInfo.substring(1));
  ...
  private void processRootDocument(HttpServletRequest request, HttpServletResponse response)
      throws ServletException, IOException {
