...
import com.nuecho.rivr.voicexml.rendering.json.JsonServletResponseContent.*;
...
/**
 * {@link StepRenderer} used for Rivr dialogue runner. All the {@link VoiceXmlOutputTurn
 * VoiceXmlOutputTurns} and {@link VoiceXmlLastTurn VoiceXmlLastTurns} are rendered as JSON object.
 *
 * @author Nu Echo Inc.
 */
public class JsonStepRenderer implements
  ...
  public ServletResponseContent createDocumentForOutputTurn(VoiceXmlOutputTurn outputTurn,
      ...
      VoiceXmlDialogueContext dialogueContext) throws StepRendererException {
    ...
    JsonUtils.add(builder, SERVLET_PATH_PROPERTY,
        VoiceXmlDomUtil.getSubmitPathForTurn(dialogueContext, outputTurn));
    JsonUtils.add(builder, TURN_PROPERTY, turn);

    ServletResponseContent voiceXmlResponseContent = mVoiceXmlStepRenderer
        .createDocumentForOutputTurn(outputTurn, request, response, dialogueContext);
  ...
  public ServletResponseContent createDocumentForLastTurn(VoiceXmlLastTurn lastTurn,
    ...
    ServletResponseContent voiceXmlResponseContent = mVoiceXmlStepRenderer
        .createDocumentForLastTurn(lastTurn, request, response, dialogueContext);
  ...
  private JsonServletResponseContent processResponse(HttpServletRequest request,
      JsonObjectBuilder builder, ServletResponseContent voiceXmlServletResponseContent)
    ...
    JsonpMode jsonpMode =
        callback != null ? textarea != null ? JsonpMode.TEXTAREA : JsonpMode.NORMAL
            : JsonpMode.DISABLED;
    try {
