...
/**
 * This abstract class is the superclass of all classes representing a turn interpreted by the
 * VoiceXML platform.
 *
 * @author Nu Echo Inc.
 * @see Interaction
...
public abstract class VoiceXmlOutputTurn extends VoiceXmlDocumentTurn implements OutputTurn {
  ...
  private static void addEventHandler(Element vxmlElement) {
    ...
    ifErrorHandlingElement.setAttribute(COND_ATTRIBUTE,
        RIVR_SCOPE_OBJECT + "." + LOCAL_ERROR_HANDLING_PROPERTY);
    ...
    setErrorHandlingScript.append(RIVR_SCOPE_OBJECT).append(".")
        .append(LOCAL_ERROR_HANDLING_PROPERTY).append("=").append(TRUE);
  ...
  protected static void addEventHandlerScript(Element parent) {
    ...
    addEventScript.append(RIVR_SCOPE_OBJECT).append(".addEventResult(").append(EVENT_NAME_VARIABLE)
        .append(", ").append(EVENT_MESSAGE_VARIABLE).append(")");

    createScript(parent, addEventScript.toString());
  }

  protected abstract void fillVoiceXmlDocument(Document document, Element formElement,
